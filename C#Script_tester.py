...
try:
    ...
    # actor_critic = Actor_Critic(input_dim, action_dim)
    # actor_critic.load_state_dict(torch.load('/Users/sargisvardanyan/PycharmProjects/Quadcopter_RL_car_detection/Actor_Critic6_att.pt'))
...
def generate_action_number(states, state_dict, b_vec, hid=None):
                    ...
                    np.where(object_lost,  # Если объект потерян
                        np.where(old_act == 1,
                            ...
                            np.where(old_act == 2,
                                ...
                                10,
                                np.random.choice([1, 2], size=bbox_w.shape, p=[0.6, 0.4])))),  # Рандомный выбор поворота
    ...
    target_vector = torch.tensor([
        ...
        [0, 1.2, -0.5, 1., -0.1, 0, 0.7],  #1_7
        [0, -0.5, 1.2, 1., -0.1, 0, 0.7],  #2_8
    ...
    # optimizer = torch.optim.AdamW(actor.parameters(), lr=0.000001)
    ...
    return actions, probs, m.log_prob(actions), m.entropy(), hid, losses.mean(), actions_targ
...
def send_data(new_states, keys, recent_actions, b_boxes, recent_rewards, recent_hid):
...
def train(keys, b_boxes, recent_rewards, recent_hid):
    ...
    for i in range(15, 17):
        ...
        critic_loss = F.huber_loss(values, rewards + (1 - done_i) * gamma * next_values.detach(), reduction='none')
    ...
    if int(time[0]) % 80 == 0 & len(loss_act) < 990:
        loss_act.append(actor_loss_mean.sum())
        loss_crit.append(critic_loss_mean.sum())
        sum_rewards.append(torch.mean(torch.stack(list(recent_rewards), dim=-1)[-10:]))
        loss_.append(loss)
        print('100: save')
    if tf:
